...
sudo git checkout T004_DemoVersion
...
sudo sh -c "echo 'WS_PORT=8888' >> $filename"
sudo sh -c "echo 'WS_HOST=${DOMAIN}' >> $filename"
sudo sh -c "echo '' >> $filename"


# DEMO

sudo sh -c "echo '# D E M O' >> $filename"
sudo sh -c "echo 'TOKEN_DEMO=XXX' >> $filename"
sudo sh -c "echo 'DEMO_VERSION=진실' >> $filename"
cd ..
